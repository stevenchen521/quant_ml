...
class TestProcessRawData(TestCase):
    def setUp(self):
        quandl.ApiConfig.api_key = "mpUko1mzXUjwMBpENDxz"
        # mydata = quandl.get("FRED/GDP")

    def test_process_ticker_data(self):
        # mydata = quandl.get("NASDAQOMX/COMP")
        mydata = quandl.get("BCII/_XNDX")
        print (mydata)
    ...
    def test_merge_data_add_nasdaq(self):
        ...
        df5 = df5[['date', 'close', 'volume']].rename(columns={'close': 'nas_close', 'volume': 'nas_volume'})
        ...
        # df_final = df_final[['open', 'high', 'low', 'close', 'volume', 'on', '1m',
        #                      '6m', '1y', 'IS_EPS', 'BEST_EPS', 'RETURN_COM_EQY', 'tp_score']]
        df_final = df_final[['open', 'high', 'low', 'close', 'volume', 'on', '6m', 'nas_close', 'nas_volume', 'tp_score']]
        df_final[['nas_close', 'nas_volume']] = df_final[['nas_close', 'nas_volume']].shift(-1)
        df_final = df_final.dropna(how='any')
        df_final.to_csv('../../data/SH_index_all.csv')